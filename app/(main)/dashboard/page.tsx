    ...
    <>
      ...
      <div className="h-fit relative flex flex-col items-center justify-center gap-7 py-14">
        <h1 className="text-7xl poppins-medium text-center flex items-center justify-center gap-4 max-md:text-5xl max-md:flex-col max-md:gap-2">
          ...
          {
            user?.firstName ? (
                <FlipWords words={[`${user?.firstName}!`, `${generateCircles(user?.firstName?.length - 1 || 0)}`]}/>
              ) : (
                <FlipWordSkeleton />
            )
          }
        ...
        <div className="flex items-center justify-between max-md:justify-center gap-5">
          ...
          <div className="flex items-center gap-5">
            <div className="relative w-full">
              ...
              <Input placeholder="Search" className="pl-10 max-w-[280px]" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
            </div>
            <DialogComponent
              text="Add details"
              userId={user?.$id}
            ...
            >
              <Button className="inline-flex animate-shimmer items-center justify-center rounded-md border border-slate-800 bg-[linear-gradient(110deg,#000103,45%,#1e2631,55%,#000103)] bg-[length:200%_100%] px-4 font-medium text-white transition-colors" onClick={() => setDialogState(true)}>Add New</Button>
            </DialogComponent>
          </div>
        </div>
          <DashboardCards userId={user?.$id} accountId={user?.accountId} searchTerm={searchTerm}/>
      </div>
      <div className="container max-w-7xl mx-auto relative">
        <footer className="p-10">
          <h1 className="text-center text-lg">Made for {user?.lastName ||'Guest'} Ji 🙌🏻</h1>
        </footer>
      </div>
    </>
  );
